...
| 2026-09-18 15:17Z | **Open, needs a recheck after the cache window**: whether the description block of the page shows the 0.2.1 text, recognisable by the requirement line "Oder ohne AppAPI, seit 0.2.0". By the row above this is expected to arrive on its own; the row exists so that a silent failure is noticed instead of assumed away | `https://apps.nextcloud.com/apps/mcp_connector` in the browser, requirements list |
