...
**Done, 2026-09-17: 0.1.14 is live in the store.** The release that ends the Nextcloud 35
blocker: until it, the published version declared `max-version="34"` and Nextcloud 35
refused the app outright. Steps 1 to 8 are rows in the table below.
...
| 2026-09-17 05:12Z | Step 7 is done for 0.1.14: the store answered `201` and the release is live. The submission ran in the page context of the signed in store session, so the token never left the browser. The first attempt answered `400 ["Signature is invalid: "]` because a placeholder had been sent instead of the signature, which is the error message the store gives for a wrong signature |
| 2026-09-17 05:13Z | Proof 1 of step 8, the app detail page: `https://apps.nextcloud.com/apps/mcp_connector` names `0.1.14` as the only version on the page, and the platform line reads Nextcloud 32 to 35. This is the proof that matters on the day of a submission, because the catalogue endpoint is cached for four hours and still listed `0.1.9` as the newest at this moment |
| 2026-09-17 05:13Z | Proof 2, the asset: `curl -sSIL` on the download URL of `v0.1.14` answers 200, and the full download earlier the same minute carried 41009 bytes with sha256 `649a902e0fe0209c7834b57876da05d477249593db858d97058841af496cc475`, the bytes that were signed |
| 2026-09-17 05:13Z | Proof 3, the image: `https://ghcr.io/v2/street1983nk/mcp_connector/manifests/0.1.14` is an OCI index with `linux/amd64` and `linux/arm64` |
| 2026-09-17 05:13Z | Proof 4, the tags: `https://ghcr.io/v2/street1983nk/mcp_connector/tags/list` ends `["0.1.9","0.1.10","0.1.11","0.1.12","0.1.13","0.1.14"]`, none rewritten |
| 2026-09-17 05:13Z | Proof 5, the author address: `authors[0].mail` of `mcp_connector` in the catalogue is `admin@infranode.dev` and `<author mail>` in `appinfo/info.xml` at tag `v0.1.14` is the same string |
| 2026-09-17 05:05Z | What this release is for: the published 0.1.13 still declared `max-version="34"`, so Nextcloud 35 refused to install the app. 0.1.14 declares 35, and the window was measured end to end against a running 35.0.0 through HaRP rather than against the release notes (`docs/nc35-evidence.md`) |
| 2026-09-17 05:23Z | Step 7 is done for 0.1.15: the store answered `201`. The release table of the app detail page names `0.1.15` for Nextcloud 32, 33, 34 and 35 within the minute |
| 2026-09-17 05:23Z | Proofs 2 to 4 for 0.1.15: the asset answers 200 and carries 41192 bytes with sha256 `46290998f7d9a02719b48863fd89ba6cd351b05f662ae9f9c38772596485a421`, the signature verifies against the certificate with `Verified OK`, the image manifest of `0.1.15` is an OCI index with `linux/amd64` and `linux/arm64`, and the tag list ends `["0.1.12","0.1.13","0.1.14","0.1.15"]` |
| 2026-09-17 05:24Z | **Open, needs a recheck after the cache window.** The description block of the app detail page still reads "Nextcloud 32 bis 34" although 0.1.15 carries "32 bis 35" in all three languages, verified inside the downloaded asset. The catalogue endpoint at the same moment still named `0.1.9` as the newest release, so both are most likely the documented four hour cache and not a rejected description. Recheck the page and `appapi_apps.json` after that window; if the text is still the old one, the store takes the description from somewhere other than the newest release and that is worth a line of its own here |
| 2026-09-18 14:57Z | The release workflow of the tag `v0.2.0` is green, run `35359294457`, and the asset carries 41973 bytes | `gh run watch 35359294457 --exit-status`, exit 0, then `gh release view v0.2.0` |
| 2026-09-18 15:00Z | The signature of 0.2.0 verifies against the merged certificate | `openssl dgst -sha512 -verify` with the public key out of `mcp_connector.crt` over the downloaded asset: `Verified OK` |
| 2026-09-18 15:12Z | 0.2.0 was deliberately NOT submitted to the store. Its description still called identity provider sign-in planned, one release after it shipped, and the store reads the manifest at upload time, so the text had to be fixed in a release of its own. 0.2.0 stays a GitHub and ghcr release; the store went straight from 0.1.15 to 0.2.1 | the description of `appinfo/info.xml` at tag `v0.2.0` against the one at `v0.2.1` |
| 2026-09-18 15:15Z | The release workflow of the tag `v0.2.1` is green, run `35361027766`, the asset carries 42612 bytes, and the manifest inside it says `0.2.1` for both `<version>` and `<image-tag>` | `gh run watch --exit-status`, exit 0, then `tar -xzOf mcp_connector-0.2.1.tar.gz mcp_connector/appinfo/info.xml` |
| 2026-09-18 15:15Z | The signature of 0.2.1 verifies against the merged certificate | `openssl dgst -sha512 -verify` over the downloaded asset: `Verified OK` |
| 2026-09-18 15:16Z | Step 7 is done for 0.2.1: the store answered `201`. The submission ran in the page context of the signed in store session | `POST /api/v1/apps/releases` from `https://apps.nextcloud.com/account/token` |
| 2026-09-18 15:17Z | Proof 1, the app detail page: `https://apps.nextcloud.com/apps/mcp_connector` names `0.2.1` for Nextcloud 32, 33, 34 and 35 within the minute | the release table of the page, read in the browser |
| 2026-09-18 15:17Z | Proof 3 and 4, the images: `manifests/0.2.0` and `manifests/0.2.1` are both OCI indexes with `linux/amd64` and `linux/arm64`, and the tag list ends `["0.1.13","0.1.14","0.1.15","0.2.0","0.2.1"]`, none rewritten | anonymous token from `ghcr.io/token`, then the manifest and tags endpoints |
| 2026-09-18 15:17Z | Proof 5, the author address: `authors[0].mail` in the catalogue is `admin@infranode.dev` and `<author mail>` at tag `v0.2.1` is the same string | `curl -sS https://apps.nextcloud.com/api/v1/appapi_apps.json`, then `git show v0.2.1:appinfo/info.xml` |
| 2026-09-18 15:17Z | **The open question of 2026-09-17 05:24Z is answered: it was the cache, and the store does take the description from the newest release.** That day the page still read "Nextcloud 32 bis 34" while the freshly uploaded 0.1.15 said "32 bis 35"; today, before the 0.2.1 upload, the page read "32 bis 35". The two parts of the page move at different speeds, and that is the lasting fact: the release table named `0.2.1` within the minute, while the description block at the same moment still carried the 0.1.15 text, without the line about running without AppAPI | the page read in the browser at both dates, compared against the description inside the uploaded asset |
| 2026-09-18 15:17Z | **Recheck was open until 15:58Z, now closed by the row below**: whether the description block of the page shows the 0.2.1 text, recognisable by the requirement line "Oder ohne AppAPI, seit 0.2.0". By the row above this is expected to arrive on its own; the row exists so that a silent failure is noticed instead of assumed away | `https://apps.nextcloud.com/apps/mcp_connector` in the browser, requirements list |
| 2026-09-18 15:58Z | **Recheck done, the cache resolved on its own**: the description block shows the 0.2.1 text in German ("Oder ohne AppAPI, seit 0.2.0") and in English ("Or without AppAPI, since 0.2.0"), and `appapi_apps.json` names `0.2.1` as the newest release, both earlier than the four hour window suggested | `GET /apps/mcp_connector/description` with `Accept-Language: de` and without, plus `curl -sS https://apps.nextcloud.com/api/v1/appapi_apps.json` |
| 2026-09-18 15:17Z | The catalogue endpoint lagged as documented: at the minute of the upload `appapi_apps.json` still listed `0.1.11` as its newest release for `mcp_connector` | `curl -sS https://apps.nextcloud.com/api/v1/appapi_apps.json` |
| 2026-09-18 15:12Z | A sixth place carries the version and no gate holds it: `uv.lock` names the project version, and it had said `0.1.13` ever since, through four releases including the tag `v0.2.0`. `uv sync` corrected it to `0.2.1` in this release. Worth a line in step 1 of the runbook, not a gate on its own | `git show v0.2.0:uv.lock` against the file at `v0.2.1` |
...
1. **Raise the version in all four places, to the same string.** `version` in
   ...
   Two more places travel along and no gate holds either. `uv.lock` names the project
   version, and it drifted unnoticed from 0.1.13 through four releases: run `uv sync`
   after the bump and commit the lock file with it. And the `Version <x>.` status line of
